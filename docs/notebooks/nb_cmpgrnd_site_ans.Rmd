...
tm_shape(yose_poibufmsk_rst_utm) +
  tm_raster(title = "Away from Existing Campsites", style = "cat", palette = c(NA, "khaki3")) +
tm_shape(yose_bnd_utm) +
  tm_borders(col = "red", lwd = 2) +
tm_grid(lines = FALSE) +
tm_layout(legend.outside = TRUE,
...
```{r chunk30}
tm_shape(campsite_ok2_rst) +
  tm_raster(style = "cat", palette = c(NA, "darkgreen")) +
tm_shape(yose_bnd_utm) +
  tm_borders(col = "red", lwd = 2) +
tm_grid(lines = FALSE) +
tm_layout(legend.show = FALSE, 
          main.title = "Areas Suitable for NEW Campsites in YNP",
          main.title.size = 1.2,
          asp = 1.2)
```
