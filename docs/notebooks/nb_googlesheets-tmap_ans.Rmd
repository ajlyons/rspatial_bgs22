...
```{r chunk13}
## Download ca_brew
ca_brew_tbl <- range_read(ss = gsheet_id, 
                               sheet = "ca_brew", 
                               range = NULL)

## Turn the data frame into a sf object, keeping only the columns we need for the popup windows
ca_brew4map_sf <- st_as_sf(ca_brew_tbl, coords = c("lon", "lat"), crs = 4326) %>% 
  select(name, address, city, state, phone)

## Create the tmap object
tm_shape(ca_brew4map_sf) +
  tm_dots(col = "blue", scale = 1.5, popup.vars = c("address", "city", "state", "phone")) +
tm_basemap(providers$OpenStreetMap)
```
